...
/**
 *  \file       tourneedelivraison_note.php
 *  \ingroup    tourneesdelivraison
 *  \brief      Car with notes on TourneeDeLivraison
 */
...
$typetournee = str_replace("_note.php","", $arr[count($arr)-1]);
...
$langs->loadLangs(array("tourneesdelivraison@tourneesdelivraison","companies"));
...
$object = new TourneeUnique_lines($db);
$object->fetch($lineid);

$extrafields = new ExtraFields($db);
$diroutputmassaction=$conf->tourneesdelivraison->dir_output . '/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array($typetournee.'note','globalcard'));     // Note that conf->hooks_modules contains array
...
if ($id > 0 || ! empty($ref)) $upload_dir = $conf->tourneesdelivraison->multidir_output[$object->entity] . "/" . $object->id;

$permissionnote=1;
//$permissionnote=$user->rights->tourneesdelivraison->creer;	// Used by the include of actions_setnotes.inc.php
...
$help_url='';
llxHeader('',$langs->trans($typenom),$help_url);
...
if ($id > 0 || ! empty($ref))
{
	...
	dol_fiche_head($head, 'note', $langs->trans('Livraison'), -1, 'tourneesdelivraison@tourneesdelivraison');
	...
	if (! empty($conf->projet->enabled))
	{
	    ...
	    if ($user->rights->tourneesdelivraison->creer)
	    {
	        if ($action != 'classify')
	...
	dol_banner_tab($object, 'lineid', $linkback, 1, 'id', 'ref_arret', $morehtmlref);
