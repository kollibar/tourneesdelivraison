	  ...
	  <td class="linecolcmde">
			...
			if( $nb[TourneeUnique_lines_cmde::DATE_NON_OK]==0 && $nb[TourneeUnique_lines_cmde::DATE_OK]==0
					...
					&& $user->rights->commande->creer) {
						...
						print '</td></tr>';
						if( empty($line->aucune_cmde)){
							print '<tr><td><div class="inline-block divButAction tourneeBoutons"><a class="butAction" href="'.DOL_URL_ROOT.'/commande/card.php?socid='.$line->fk_soc.'&amp;action=create&amp;datedelivery='.$parent->date_tournee.'">'.$langs->trans("AddOrder").'</a></div>
								</td></tr>';
						}
			} ?>
			</table>
