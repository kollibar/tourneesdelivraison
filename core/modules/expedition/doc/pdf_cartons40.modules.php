 ...
 */
class pdf_cartons40 extends ModelePdfExpedition
{
     /**
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "cartons40";
		$this->description = $langs->trans('PDFCartons40Description');
  ...
  function _write_case(&$pdf, $pos, $url, $thirdparty, $num, $tot, $qty, $product_id, $outputlangs, $default_font_size){
    ...
    $pdf->MultiCell($this->largeur, 1, $product->label, 0, 'L');
...
/*
  ...
  /**
    ...
    $this->posxpicture=$this->posxweightvol;
        ...
        $url=$_SERVER["PHP_SELF"]."/expedition/card.php?id=$id";
        ...
        foreach ($object->lines as $line) {
          ...
          if( !empty($product->array_options['options_colisage'])){
            $nbColis += (int)($line->qty_shipped/$product->array_options['options_colisage']);
            $nbColis += ($line->qty_shipped%$product->array_options['options_colisage'] != 0);
          } else $nbColis++;
        }
