...
/**
 *	\file       htdocs/core/modules/expedition/doc/pdf_bordereauLivraison.modules.php
 *	\ingroup    expedition
 *	\brief      Fichier de la classe permettant de generer les bordereaux envoi au modele bordereauLivraison
 */
...
/**
 *	Classe permettant de generer les borderaux envoi au modele bordereauLivraison
 */
...
//~ 		$this->description = $langs->trans("DocumentModelStandardPDF");
		$this->description = $this->description = "Bordereau de Livraison avec colisage";
		...
		$this->posxweightvol=$this->page_largeur - $this->marge_droite - 104;
		$this->posxqtyordered=$this->page_largeur - $this->marge_droite - 80;
		$this->posxqtytoship=$this->page_largeur - $this->marge_droite - 52;
		$this->posxcolis=$this->page_largeur - $this->marge_droite  - 24;
		...
		if (!empty($conf->global->MAIN_PDF_SHIPPING_DISPLAY_AMOUNT_HT)) {
			...
			$this->posxqtytoship=$this->page_largeur - $this->marge_droite - 77;
			$this->posxcolis=$this->page_largeur - $this->marge_droite  - 58;
			$this->posxpuht=$this->page_largeur - $this->marge_droite - 39;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
        ...
        if (! empty($conf->global->MAIN_GENERATE_SHIPMENT_WITH_PICTURE))
        {
            ...
            for ($i = 0 ; $i < $nblignes ; $i++)
            {
				...
				$objphoto->fetch_optionals();

			if( $objphoto->array_options['est_cache_bordereau_livraison'] == 1 ) continue;
		...
		if ($conf->expedition->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				if (! empty($object->thirdparty->array_options['options_horaires_livr']))
				{
					...
					$pdf->writeHTMLCell(60, 4, $this->posxdesc-1, $tab_top-1, "Infos livraisons : " . $object->tracking_number, 0, 1, false, true, 'L');

					$tab_top_alt = $pdf->GetY();
					//$tab_top_alt += 1;

					$pdf->SetFont('','', $default_font_size - 1);   // Dans boucle pour gerer multi-page
					$pdf->writeHTMLCell(190, 3, $this->posxdesc-1, $tab_top_alt, dol_htmlentitiesbr($object->thirdparty->array_options['options_horaires_livr']), 0, 1);
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					$objLine = new Product($this->db);
					$objLine->fetch($object->lines[$i]->fk_product);
					$objLine->fetch_optionals();

					if( $objLine->array_options['options_est_cache_bordereau_livraison'] == 1 ) continue;  // si lien caché on passe à la suivante
					...
					$pdf->MultiCell(($this->posxcolis- $this->posxqtytoship), 3, $object->lines[$i]->qty_shipped,'','C');

					$pdf->SetXY($this->posxcolis, $curY);
					if( $objLine->array_options['options_colisage'] != 0 && $objLine->array_options['options_colisage'] != null ) {

						//if(! empty($objLine->array_options['options_codecarton']))  $code="     ".$objLine->array_options['options_codecarton'];
						//else $code=" ";
						$code="     ".$objLine->ref;
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		// calcul du nb total de colis
		$totalColis=0;
		$colisageInconnu=false;
		$colisageNonPein=false;
		...
		for ($i = 0; $i < count($object->lines); $i++){
			...
			if( $product->array_options['options_est_cache_bordereau_livraison'] == 1) continue;
			$totalToShip+=$object->lines[$i]->qty_shipped;
			$totalOrdered+=$object->lines[$i]->qty_asked;
			if( $product->array_options['options_colisage'] != 0 && $product->array_options['options_colisage'] != null ){
				if( intval($object->lines[$i]->qty_shipped) % intval($product->array_options['options_colisage']) != 0 ){
					$colisageNonPlein=true;
					$totalColis += intval(intval($object->lines[$i]->qty_shipped)/intval($product->array_options['options_colisage']))+1;
				}else $totalColis += intval($object->lines[$i]->qty_shipped)/intval($product->array_options['options_colisage']);
			}
			else $colisageInconnu=true;
		}

		$totalColis=strval($totalColis);
		if( $colisageInconnu ) $totalColis.=" + ?";
		if( $colisageNonPlein ) $totalColis.="*: attention certain colis non pleins";
    	...
    	$pdf->MultiCell($this->posxcolis - $this->posxqtytoship, $tab2_hl, $totalToShip, 0, 'C', 1);

	$pdf->SetXY($this->posxcolis, $tab2_top + $tab2_hl * $index);
    	$pdf->MultiCell($this->posxpuht - $this->posxcolis, $tab2_hl, $totalColis, 0, 'C', 1);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(($this->posxcolis - $this->posxqtytoship), 2, $outputlangs->transnoentities("QtyToShip"),'','C');
		}
		$pdf->line($this->posxcolis-1, $tab_top, $this->posxcolis-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		// ajout d'un code transport
		//$idDest=$object->thirdparty;
		//$objDest = new Societe($this->db);
		//$objDest->fetch($object->thirdparty);
