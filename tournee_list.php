...
$typetournee = str_replace("_list.php","", $arr[count($arr)-1]);
...
$help_url='';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("TourneeDeLivraisons"));
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/tourneesdelivraison/'.$typetournee.'_card.php', 1).'?id='.$id);
	exit;
}
...
if (in_array('builddoc',$arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords))
{
	...
	$genallowed=$user->rights->tourneesdelivraison->{$typetournee}->ecrire;
	$delallowed=$user->rights->tourneesdelivraison->{$typetournee}->ecrire;

	print $formfile->showdocuments('massfilesarea_tourneesdelivraison','',$filedir,$urlsource,0,$delallowed,'',1,1,0,48,1,$param,$title,'','','',null,$hidegeneratedfilelistifempty);
}
