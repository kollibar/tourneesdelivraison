...
// Get parameters
$fromQRcode=GETPOSTISSET('fromqrcode');

if( $fromQRcode ){
    $id= GETPOST('leltid','int');
    $hash=GETPOST('hash','alpha');
    $carton=GETPOST('carton','int');
} else {
  $id = GETPOST('id', 'int');
...
$object=new TourneeUnique_lines_cmde_elt($db);
$object->fetch($id,'');
$object->fetch_thirdparty();

if( $object->type_element == 'shipping' ){
    $exp=new Expedition($db);
    $exp->fetch($object->fk_elt);
}


$lcmde=new TourneeUnique_lines_cmde($db);
$lcmde->fetch($object->fk_tournee_lines_cmde);

$tl=new TourneeUnique_lines($db);
$tl->fetch($lcmde->fk_tournee_lines);

$tourneeunique=new TourneeUnique($db);
$tourneeunique->fetch($tl->fk_tournee);


$arret = new TourneeUnique_Arret($db);
$arret->remplit($object, $exp, $lcmde, $tl, $tourneeunique);


if( $fromQRcode && $hash != $object->getHash() ){ // erreur hash
  // A FAIRE
  exit();
}
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->rights->tourneesdelivraison->tourneeunique->lire;
	$permissiontoadd = $user->rights->tourneesdelivraison->tourneeunique->ecrire; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->rights->tourneesdelivraison->tourneeunique->effacer;
	$permissionnote = $user->rights->tourneesdelivraison->tourneeunique->ecrire; // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->rights->tourneesdelivraison->tourneeunique->ecrire; // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
if( ! array_key_exists('tourneesdelivraison', $conf) || ! $conf->qrcodescanneur->enabled ){ // si le module qrcodescanneur est activé
	accessforbidden();
}
if (!$permissiontoread) {
	accessforbidden();
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/tourneesdelivraison/tourneesdelivraison_card.php', 1).'?id=' . $tl->fk_tournee . '&mode=portable'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToTourneeUnique").'</a>';
	...
	dol_banner_tab($arret, 'ref', $linkback, 1, 'ref', 'ref', $morehtmlref);
